...
class TAC(nn.Module):
    ...
    def forward(self, input):
        ...
        group_mean = self.TAC_mean(group_mean).unsqueeze(1).expand(batch_size * T, G,
                                                                   group_mean.shape[-1]).contiguous()  # B*T, G, H
...
class BSNet(nn.Module):
    ...
    def forward(self, input):
        ...
        band_output = self.band_rnn(input.view(B * nch * self.nband, self.feature_dim, -1)).view(B * nch, self.nband,
                                                                                                 -1, T)
...
class Separator(nn.Module):
    def __init__(self, sr=44100, win=2048, stride=512, feature_dim=128, num_repeat_mask=4, num_repeat_map=2,
                 num_output=4):
        ...
        for i in range(self.nband):
            self.mask.append(nn.Sequential(nn.GroupNorm(1, self.feature_dim, torch.finfo(torch.float32).eps),
                                           ...
                                           nn.Tanh(),
                                           nn.Conv1d(self.feature_dim * 1 * self.num_output,
                                                     self.feature_dim * 1 * self.num_output, 1, groups=self.num_output),
                                           nn.Tanh(),
                                           nn.Conv1d(self.feature_dim * 1 * self.num_output,
                                                     self.band_width[i] * 4 * self.num_output, 1,
            ...
            self.map.append(nn.Sequential(nn.GroupNorm(1, self.feature_dim, torch.finfo(torch.float32).eps),
                                          ...
                                          nn.Tanh(),
                                          nn.Conv1d(self.feature_dim * 1 * self.num_output,
                                                    self.band_width[i] * 4 * self.num_output, 1, groups=self.num_output)
                                          )
                            )
    ...
    def forward(self, input):
        ...
        for i in range(len(self.band_width)):
            subband_feature_mask.append(
                self.BN_mask[i](subband_spec_RI[i].view(batch_size * nch, self.band_width[i] * 2, -1)))
        subband_feature_mask = torch.stack(subband_feature_mask, 1)  # B, nband, N, T

        subband_feature_map = []
        for i in range(len(self.band_width)):
            subband_feature_map.append(
                self.BN_map[i](subband_spec_RI[i].view(batch_size * nch, self.band_width[i] * 2, -1)))
        subband_feature_map = torch.stack(subband_feature_map, 1)  # B, nband, N, T
        # separator
        sep_output = checkpoint_sequential(self.separator_mask, 2,
                                           subband_feature_mask.view(batch_size, nch, self.nband * self.feature_dim,
                                                                     -1))  # B, nband*N, T
        ...
        sep_output2 = checkpoint_sequential(self.separator_map, 2,
                                            combined3.view(batch_size, nch, self.nband * self.feature_dim,
                                                           -1))  # 1B, nband*N, T
        ...
        for i in range(self.nband):
            ...
            ##################################
            this_output2 = self.map[i](sep_output2[:, i]).view(batch_size * nch, 2, 2, self.num_output,
                                                               self.band_width[i], -1)
